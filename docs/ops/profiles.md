...
자동 검사 도구는 대표 슬라이드의 `base_slide.elements[].element` 값으로
아래 유형을 구분.
...
`song_lyric`은 `Themes/찬양+성가`, `sermon`은 `Themes/말씀` 사용.
여러 디자인이 섞였거나 빈 슬라이드는 자동으로 바꾸지 않고 사람이 관리.

공통 배경은 `Media/Assets/bg_worship_2.png`, 사도신경은
`apostles_creed_01.png`부터 `03.png`까지 사용.
