...
최대 60회 확인. `401`·`403` 접근 거부도 최대 1분 확인하고 계속되면
중단한 뒤 서버 프록시와 WebDAV 권한 확인. 예약 작업은 현재 정의와 다를
...
`PP-SessionWatcher`의 호환 진입점은 실제 감시기가 끝날 때까지 대기. 같은
로그인에서 중복 실행되지 않도록 예약 작업의 `IgnoreNew` 설정 사용.
